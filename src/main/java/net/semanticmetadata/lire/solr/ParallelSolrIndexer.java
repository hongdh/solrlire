    ...
    class Producer implements Runnable {
        public void run() {
            try {
                ...
                String file = null;
                File next = null;
                String[]fileurls=new String[20]; 
                
                while ((file = br.readLine()) != null) {
//                	fileurls = file.split("\t",2);
//                  next = new File(fileurls[0]);
                    try {
...
//                        String path = next.getCanonicalPath();
                        //images.put(new WorkItem(fileurls[1], buffer));
                    	byte[] buffer = null;
                    	images.put(new WorkItem(file,buffer)); 
                    } catch (Exception e) {
...
//        String urlString = "http://192.168.72.13:8080/solr";
...
//                        locallyEnded = true;
                    ...
                    if (!locallyEnded) {
                        ...
                        String fileinfo = tmp.getFileName();
                        fileurls = fileinfo.split("\t",2);
                        String fileUrl = fileurls[1];
                        String fileid = fileurls[0];
                        ...
                        //document.addField("id", fileid);
                        //document.addField("url", fileUrl);
                        
                        sb.append("<doc>");
                        sb.append("<field name=\"id\">");
                        if (idp == null)
                            sb.append(fileid);//tmp.getFileName()
                        else
                            sb.append(idp.getIdentifier(fileid));
                        sb.append("</field>");
                        sb.append("<field name=\"url\">");
                        if (idp == null)
                            sb.append(fileUrl);
                        else
                            sb.append(idp.getTitle(fileUrl));
                        sb.append("</field>");
                        if (idp != null)
                            sb.append(idp.getAdditionalFields(fileUrl));
...
//                        solr.commit();
                        ...
                        if (!individualFiles) {
                            ...
                            OutputStream mos = new BufferedOutputStream(new FileOutputStream(fileid + "_solr.xml"));
